...
const formatTelegramMessage = (payload: EmailPayload): string => {
  const lines = [
    '<b>New lead</b>',
    `<b>Source:</b> ${escapeHtml(toCleanString(payload.source) || 'Site')}`,
    '',
  ];
  ...
  pushField('Phone', payload.phone);
  pushField('Email', payload.email);
  pushField('Telegram', payload.telegram);
  pushField('City', payload.city);
  pushField('Date', payload.date);
  pushField('Format', payload.format);
  pushField('Comment', payload.comment);
  ...
  if (payload.extra) {
    ...
    if (extraEntries.length > 0) {
      lines.push('');
      lines.push('<b>Calculated fields:</b>');
...
const formatEmailFailureAlertMessage = ({
  ...
  const lines = [
    ...
    '',
    `<b>Source:</b> ${escapeHtml(toCleanString(source) || 'Site')}`,
    `<b>Name:</b> ${escapeHtml(toCleanString(name) || '-')}`,
    `<b>Phone:</b> ${escapeHtml(toCleanString(phone) || '-')}`,
  ];
  ...
  lines.push('');
  lines.push(`<b>Reason:</b> ${escapeHtml(toCleanString(errorMessage) || 'Unknown error')}`);
  return lines.join('\n');
};
