...
const AdminGearButton = () => {
  ...
  const { isEditing, toggle: toggleEditMode } = useEditMode();
      ...
      <div className="fixed bottom-5 right-5 z-[9999] flex items-center gap-2">
        {isAuthenticated && isAdmin ? (
          <button
            ...
            className={`flex h-10 w-10 items-center justify-center rounded-full border shadow-xl transition ${
              isEditing
                ? 'border-amber-400/60 bg-amber-500/20 text-amber-300'
                : 'border-white/20 bg-slate-800 text-slate-400 hover:border-amber-400/50 hover:text-amber-300'
            }`}
            title={isEditing ? 'Exit inline edit mode' : 'Enter inline edit mode'}
            aria-pressed={isEditing}
          >
            {isEditing ? <PencilOff className="h-5 w-5" /> : <Pencil className="h-5 w-5" />}
          </button>
        ) : null}
